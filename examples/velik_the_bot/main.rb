...
TrovoBot.start do |chat, channel_id|   # this is designed for a multichannel bot it's yet a singleton
  ...
  #   msg[:data][:chats].each do |chat|
      ...
      when 0
        ...
        when /\A\\access\s+(quote|bet)\z/
          TrovoBot::queue.push ["#{chat[:nick_name]}'s current access level: #{get_level[chat[:sender_id], channel_id, $1]}", channel_id]
        when /\A\\access\s+(quote|bet)\s+(\S+)\z/
          TrovoBot::queue.push ["#{$2}'s current access level: #{get_level[TrovoBot::name_to_id($2), channel_id, $1]}", channel_id]
        when /\A\\access\s+(quote|bet)\s+(\S+)\s+([+-])\z/
          cmd, name, dir = $1, $2, $3
          ...
          db.transaction{ |tr| tr["access.#{cmd}.#{channel_id}.#{id}"] = dir == ?+ ? 1 : 0 }
          TrovoBot::queue.push ["#{name}'s new access level: #{get_level[id, channel_id, cmd]}", channel_id]
